                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <tr v-for="(complaint, x) in complaints.data" :key="complaint.id">
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-primary btn-sm btn-toggle-custom" @click="editModal(complaint)" :disabled="!$gate.hasPermission('can_edit_complaint_type')">Edit</button>
                                                <button type="button" class="btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                    <span class="sr-only">Toggle Dropdown</span>
                                                </button>
                                                <div class="dropdown-menu">
                                                    <a href="javascript:void(0);" @click="deleteState(complaint.id)"
                                        ...
                                        <td class="text-capitalize">{{ complaint.atm_atm_id }}</td>
                                        <td class="text-capitalize">{{ complaint.docket_no }}</td>
                                        <td class="text-capitalize">{{ complaint.docket_no }}</td>
                                        <td class="text-capitalize">{{ complaint.created_at }}</td>
                                        <td class="text-capitalize">{{ complaint.work_status }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
export default {
    ...
    async submitForm() {
      ...
      try {
        ...
        async export_csv() {
            const response = await axios.get('api/complaint?export=1', {
                params: {
                    search: this.search
                }
            });
        ...
        getResults(page = 1) {
            ...
            axios.get('api/admin/complaint/list', {
                params: {
                    page: page,
                    search: this.search
                }
        ...
        deleteState(id) {
            Swal.fire({
                ...
                if (result.value) {
                    this.form.delete('api/complaint/' + id).then((response) => {
        ...
        loadStates() {
            ...
            axios.get('api/admin/complaint/list').then(({ data }) => {
                console.log("Complaint data: ", data);
                this.complaints = data.data.complaints;
                console.log("statelist",this.complaints);
